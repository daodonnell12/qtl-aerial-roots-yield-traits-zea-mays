...
Evolution <- read.csv("2024_Evolution_Metadata_Averages_Final_for-R-figure_v4.csv") #new file excluding outliers from spatial effects, and with ind. check plots, ordered by group and then alphabetical order, including cultivar 5A
Evolution
print(Evolution)
...
Zea_Trial_1 <- read.csv("2024_Evolution_Metadata_Averages_Final_for-R-figure_v4.csv")
...
plot_NDFA_T1_B73_1 <- ggplot(Zea_Trial_1,aes(x=Name,y=NDFA_T1_B73.ref_Mean,col=Group))+geom_point() +
  ...
  ##if v4, then below
  scale_x_discrete(labels = c("Jala, Nayarit", "Jaltepec de Candayoc", "Lowland cultivar 5A", "San Cristobal Lachirioag", "San Ildefonso Villa Alta", "San Jose Chinantequilla", "San Juan Comaltepec", "San Marcos Moctum", "Santa Maria Temaxcalapa", "Santiago Choapam", "Santiago Tepitongo", "San Miguel Metepec", "Santa Maria Tiltepec", "Totontepec cultivar 1", "Totontepec cultivar 2", "Tototepec cultivar 3", "Venustiano Carranza, DG", "Z. diploperennis", "Z. mays L. spp. mexicana", "Z. mays L. spp. parviglumis", "B73", "Hickory King", "Mo17", "B73 x Mo17 (1A)", "B73 x Mo17 (1D)", "B73 x Mo17 (3C)", "B73 x Mo17 (5B)", "B73 x Mo17 (7A)", "B73 x Mo17 (7D)", "B73 x Mo17 (Avg.*)")) +
